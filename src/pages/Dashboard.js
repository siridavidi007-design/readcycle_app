  ...
  useEffect(() => {
    if (role === "student") {
      ...
      approvedRequests.forEach(request => {
        ...
        if (daysDiff <= 10 && daysDiff >= -60) { // Show reminders from 10 days before to 60 days overdue
          let message = "";
          let urgency = "normal";
          let showReminder = false;
          
          if (daysDiff > 0) {
            // Show reminders at 10, 5, and 1 days before
            if (daysDiff === 10 || daysDiff === 5 || daysDiff === 1) {
              showReminder = true;
            ...
            // Show overdue reminders every 2 days after due date
            const daysOverdue = Math.abs(daysDiff);
            if (daysOverdue % 2 === 0) { // Every 2 days
              showReminder = true;
    ...
    <div className="container">
      ...
      {role === "student" && (
        <>
          ...
          {/* Chapter Events - Enhanced for Chapter Leaders */}
          <div className="card">
            ...
            {events.length === 0 ? (
              ...
              <div>
                {role === "chapterLeader" ? (
                  // Chapter leader view with delete buttons
                        ...
                        {events.map((e) => (
                          <tr key={e.id}>
                            <td>
                              ...
                              <br />
                              <small style={{ color: '#666' }}>{e.description}</small>
                            </td>
                            <td>{e.date}</td>
                            <td>{e.location}</td>
                            <td>
